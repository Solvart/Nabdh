...
const RequesterAuth: React.FC = () => {
    ...
    const handleLogin = (e: React.FormEvent) => {
        e.preventDefault();
        if (login(identifier, password, 'requester')) {
            navigate('/dashboard');
        } else {
    ...
    const handleSignup = (e: React.FormEvent) => {
        ...
        if (signup({ ...signupData, username: formData.email.split('@')[0] }, 'requester')) {
            navigate('/dashboard');
        }
    };
        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 px-4">
            <div className="max-w-md w-full space-y-8">
                <div className="text-center">
                    ...
                    <h2 className="mt-6 text-3xl font-extrabold text-gray-900 dark:text-gray-100">{t('auth.reqSpaceTitle')}</h2>
                </div>
