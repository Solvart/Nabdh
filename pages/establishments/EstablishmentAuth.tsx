...
const EstablishmentAuth: React.FC = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        password: '',
        confirmPassword: '',
        type: 'Hôpital' as 'Hôpital' | 'Clinique' | 'Banque_de_Sang',
        phone: '',
        wilaya: 'Alger',
    ...
    const handleLogin = (e: React.FormEvent) => {
        e.preventDefault();
        if (login(identifier, password, 'establishment')) {
            navigate('/dashboard');
        } else {
    ...
    const handleSignup = (e: React.FormEvent) => {
        ...
        if (signup({ ...signupData, username: formData.email.split('@')[0] }, 'establishment')) {
            navigate('/dashboard');
        }
    };
        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 px-4">
            <div className="max-w-md w-full space-y-8">
                <div className="text-center">
                    ...
                    <h2 className="mt-6 text-3xl font-extrabold text-gray-900 dark:text-gray-100">{t('auth.estSpaceTitle')}</h2>
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 p-8 shadow-lg rounded-lg">
                    ...
                    {isLogin ? (
                        ...
                        <form className="space-y-4 mt-6" onSubmit={handleSignup}>
                            <Input id="name" name="name" label={t('auth.estNameLabel')} value={formData.name} onChange={handleInputChange} required />
                            ...
                            <Input id="address" name="address" label={t('auth.addressLabel')} value={formData.address} onChange={handleInputChange} required />
